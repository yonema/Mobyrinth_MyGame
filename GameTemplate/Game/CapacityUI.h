	...
	*/
	namespace nsUI
	{
		...
		private:	//データメンバ
			...
			nsGraphic::nsSprite::CSpriteRender* m_capacityDisplaySR = nullptr;	//キャパシティを表示するスプライト
			nsGraphic::nsFont::CFontRender* m_RONumFR[enFrontAndBackSideNum];	//表側と裏側にあるアイテムの数のフォント
			nsGraphic::nsFont::CFontRender* m_capacityFR[enFrontAndBackSideNum];	//キャパシティのフォント
			...
			nsSound::CSoundCue* m_capacity_over = nullptr;			//キャパオーバーの音
