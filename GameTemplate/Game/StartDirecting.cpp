	...
	namespace nsCamera
	{
		...
		*/
		void CStartDirecting::GoAroundStage()
		{
			...
			{
				//タイマーを進める
				m_firstWaitTimer += nsTimer::GameTime().GetFrameDeltaTime();
				return;
			}
			...
			m_goAroundStageAngle += ANGLE_MAX_GO_AROUND_STAGE / TIME_GO_AROUND_STAGE *
				nsTimer::CGameTime::GetInstance().GetFrameDeltaTime();
			//クォータニオンに角度をセットする
		...
		void CStartDirecting::FadeDirectingCamera()
		{
			...
			else if (m_fadeTimer < TIME_FADE_WAIT)
			{
				...
				g_graphicsEngine->GetHUD().
					SetFadeSpriteAlphaValue(
						nsGraphic::nsSprite::spriteRenderConstData::ALPHA_VALUE_OPACITY
					);
			}
			//フェードインの時間か？
			...
			else
			{
				...
				g_graphicsEngine->GetHUD().
					SetFadeSpriteAlphaValue(
						nsGraphic::nsSprite::spriteRenderConstData::ALPHA_VALUE_TRANSPARENT
					);
				//フェードを行わないようにする
				m_fadeFlag = false;
			}
		...
		*/
		void CStartDirecting::FallPlayer()
		{
			...
			if (m_player == nullptr)
			{
				//見つかっていない
				//プレイヤーを探す
				m_player = FindGO<nsPlayer::Player>(
					nsCommonData::GetGameObjectName(nsCommonData::EN_GO_TYPE_PLAYER)
					);
				//まだ見つかっていなかったら
				if (m_player == nullptr)
				{
			...
			//落下させる	//デルタタイムを掛ける
			fallpos.y -= SPEED_FALL_PLAYER * nsTimer::GameTime().GetFrameDeltaTime();
			//高さが地面より低いか？
			if (fallpos.y < HEIGHT_GROUND)
			{
				...
				{
					//落下後に少し待機する
					m_waitTimerAfterFell += nsTimer::GameTime().GetFrameDeltaTime();
				}
				//少し待った後
				else
				{
					...
					//ポーズの操作フラグをtrueにする。
					nsUI::CPause* pause = FindGO<nsUI::CPause>(
						nsCommonData::GetGameObjectName(nsCommonData::EN_GO_TYPE_PAUSE)
						);
					pause->SetCanPause(true);
					//UFOがあったら動かす
					nsLevelObject::nsObstacleObject::CUFO* ufo = 
						FindGO<nsLevelObject::nsObstacleObject::CUFO>(
						nsCommonData::GetGameObjectName(nsCommonData::EN_GO_TYPE_UFO)
						);
					if (ufo)
						ufo->SetMoveSpeed();
				}
