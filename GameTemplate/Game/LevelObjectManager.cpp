	...
	namespace nsLevelObject
	{
		...
		*/
		...
		(const int rpIndex, const Vector3& pos, const float dist, const bool leftOrRight, int* pNextIndex)
		{
			...
			int nextIndex = 0;
			//左右どちらに飛ばすか？
			if (leftOrRight == nsCommonData::EN_RIGHT)
			{
				//右に飛ばす
				nextIndex = rpIndex;
			}
		...
		*/
		...
		{
			//表側か裏側か
			int frontOrBackSide = nsCommonData::EB_NONE_SIDE;

			//左側のウェイポイントをしきい値と比べて、表側か裏側か調べる
			if (m_frontOrBackSideThresholdSmall <= lpIndex && lpIndex <= m_frontOrBackSideThresholdBig)
			{
				//裏側
				frontOrBackSide = nsCommonData::EN_BACK_SIDE;
			}
			else
			{
				//表側
				frontOrBackSide = nsCommonData::EN_FRONT_SIDE;
			}
