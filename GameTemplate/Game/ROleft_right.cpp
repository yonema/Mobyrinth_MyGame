		...
		*/
		namespace nsReversibleObject
		{
			...
			void ROleft_right::QuerySub()
			{
				...
				//障害オブジェクトの「一方通行」をクエリ
				QueryLOs<nsObstacleObject::OOoneway>(
					EN_OO_TYPE_ONEWAY, [&](nsObstacleObject::OOoneway* oneway) -> bool
					{
						//「一方通行」と衝突した時の処理
						HitOneway(oneway);
			...
			*/
			void ROleft_right::HitOneway(nsObstacleObject::OOoneway* oneway)
			{
				//行動できるか？
				if (m_actionFlag)
				{
					...
					if (IsHitLevelObject(*this, *oneway))
					{
						//右か左か？
						//デフォは左
						nsCommonData::EN_LEFT_OR_RIGHT leftOrRight = nsCommonData::EN_LEFT;

						//自身が「右」の時
						if (GetObjectType() == EN_RO_TYPE_RIGHT)
							//右にする
							leftOrRight = nsCommonData::EN_RIGHT;
