		...
		*/
		namespace nsObstacleObject
		{
			...
			*/
			...
			{
				//ゴール時のエフェクトを生成
				m_goalEF = NewGO<Effect>(nsCommonData::PRIORITY_FIRST);
				//初期化
			...
			*/
			...
			{
				//ゴールのスプライトの生成
				m_goalSR = NewGO<nsGraphic::nsSprite::CSpriteRender>(nsCommonData::PRIORITY_SECOND);
				//初期化
				m_goalSR->Init(
					SPRITE_FILEPATH_GOAL,
					SPRITE_WIDHT_GOAL,
					SPRITE_HEIGHT_GOAL,
					nsGraphic::nsSprite::spriteRenderConstData::SPRITE_PIVOT_DEFALUT,
			...
			*/
			...
			{
				//ゴール時のサウンドを生成する
				m_goalSC = NewGO<nsSound::CSoundCue>(nsCommonData::PRIORITY_FIRST);
