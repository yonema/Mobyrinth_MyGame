...
namespace nsMobyrinth
{
	...
	*/
	namespace nsTitle
	{
		...
		{
			//背景の生成と初期化
			m_stageTitle = NewGO<nsStage::CStage_title>(PRIORITY_FIRST);
			//開始演出をしないようにする
		...
		*/
		void CTitle::InitTitleSprite()
		{
			...
			//タイトル画面の表示
			m_level2D.Init(LEVEL_2D_FILEPATH_TITLE, [&](nsLevel2D::Level2DObjectData& objdata)
				{
					//名前が一致でフックする
		...
		*/
		void CTitle::InitStageSelectSprite()
		{
			...
			//レベル2Dの初期化
			m_level2D.Init(LEVEL_2D_FILEPATH_STAGE_SELECTION, [&](nsLevel2D::Level2DObjectData& objdata)
				{
					//名前が一致でフックする
		...
		*/
		...
		void CTitle::StageDecision()
		{
			...
			nsStage::CStage* stage = nullptr;	//ステージのポインタ
			...
			{
				//ステージを生成
				stage = NewGO<nsStage::CStage>(PRIORITY_FIRST);
				...
				if (m_stageSelectState == EN_STAGE_4)
				{
					...
					stage->SetSwitchTime(nsStage::stageBaseConstData::SWITCH_TIME_STAGE_4);
				}
			}
