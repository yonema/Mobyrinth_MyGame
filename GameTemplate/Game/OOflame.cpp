		...
		*/
		namespace nsObstacleObject
		{
			...
			bool OOflame::StartSub()
			{
				...
				//OBBのタグを設定する
				SetOBBTag(nsOBB::COBB::EN_OO_TYPE_FLAME);
			...
			*/
			...
			{
				//ポイントライトを生成
				m_pointLight = NewGO<nsLight::CPointLight>(nsCommonData::PRIORITY_FIRST);
			...
			*/
			void OOflame::InitSound()
			{
				...
				//炎の常時鳴っているサウンドを生成する
				m_flameSE = NewGO<CSoundCue>(nsCommonData::PRIORITY_FIRST);
				...
				//炎と水の反応サウンドを生成する
				m_flame_waterSE = NewGO<CSoundCue>(nsCommonData::PRIORITY_FIRST);
			...
			*/
			...
			{
				//炎と水の反応エフェクト作成
				m_flame_waterEF = NewGO<Effect>(nsCommonData::PRIORITY_FIRST);
			...
			void OOflame::Damage()
			{
				...
				else
				{
					...
					//モデルのサイズをHPに応じて小さくする
						...
						//だったらうまくいかない。
					SetScale(g_VEC3_ONE * 
						(static_cast<float>(m_hp + SIZE_COMPLEMENT_FLAME) /
							static_cast<float>(HP_MAX + SIZE_COMPLEMENT_FLAME)));
				}
