	...
	namespace nsGraphic
	{
		...
		*/
		namespace nsHUD
		{
			...
			{
				//デルタタイムをセットしておく
				m_deltaTime = nsTimer::GameTime().GetFrameDeltaTime();
			...
			void CHUD::InitSprite()
			{
				...
				//シェーダーを設定
				afterWipingSpriteInitData.m_fxFilePath = 
					nsGraphic::nsSprite::spriteRenderConstData::SHADER_FILEPATH_DEFAULT;
				//テクスチャの幅と高さを設定
				...
				//シェーダーを設定
				backSpriteInitData.m_fxFilePath = 
					nsGraphic::nsSprite::spriteRenderConstData::SHADER_FILEPATH_DEFAULT;
				//テクスチャの幅と高さを設定
				...
				m_backSprite.Update(
					...
					nsGraphic::nsSprite::spriteRenderConstData::SPRITE_SCALE_DEFAULT
				);
			...
			void CHUD::InitFade()
			{
				...
				//シェーダーのファイルパス
				fadeSpriteInitData.m_fxFilePath = 
					nsGraphic::nsSprite::spriteRenderConstData::SHADER_FILEPATH_DEFAULT;
			...
			void CHUD::UpdateParam()
			{
				...
				if (m_wipeFlag)
				{
					...
					//ワイプアウト時のデルタタイムを使う。
					if (m_wipeParam.inOrOut == nsCommonData::EN_OUT)
						m_deltaTime = nsTimer::GameTime().GetFrameDeltaTime();
