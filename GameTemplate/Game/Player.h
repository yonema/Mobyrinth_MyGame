...
{
	namespace nsCamera { class CGameCamera; }
	namespace nsLevelObject { namespace nsReversibleObject { class CReversibleObject; } }
	...
	namespace nsPlayer
	{
		...
		public:		//メンバ関数
			...
			*/
			void SetHoldObject(
				const bool holdFlag,
				nsLevelObject::nsReversibleObject::CReversibleObject* reversibeObject = nullptr)
			{
			...
			*/
			const nsLevelObject::nsReversibleObject::CReversibleObject* GetReversibleObject() const
			{
			...
			*/
			nsLevelObject::nsReversibleObject::CReversibleObject* GetReversibleObject()
			{
				return m_reversibleObject;
			}
		...
		private:	//データメンバ
			...
			*/
			...
			nsGraphic::nsModel::CModelRender* m_goalPlayerMR = nullptr;	//ゴール時のプレイヤーのモデル
			//ゴール時のアニメーションクリップ。
			...
			* 移動関連
			*/
			nsOBB::CMyCharacterController m_myCharaCon;	//myキャラクターコントローラー
			...
			*/
			...
			nsCommonData::EN_LEFT_OR_RIGHT m_leftOrRight = nsCommonData::EN_LEFT;//キャラクターの左右の向き
			//持っている反転オブジェクトのポインタ
			nsLevelObject::nsReversibleObject::CReversibleObject* m_reversibleObject = nullptr;
			...
			* スタン関連
			*/
			bool m_stunLeftOrRight = nsCommonData::EN_LEFT;				//スタン中の吹っ飛ぶ方向
			...
			const nsOBB::COBB* m_hitOBB = nullptr;					//衝突したOBBのポインタ
			...
			* SE関連
			*/
			nsSound::CSoundCue* m_fallstartSE = nullptr;	//落ちるときのサウンドキュー
			int m_fallcount = 0;				//落ちるときのカウンター
			nsSound::CSoundCue* m_walkSE = nullptr;		//歩くときのサウンドキュー
			nsSound::CSoundCue* m_runSE = nullptr;		//走るときのサウンドキュー
			nsSound::CSoundCue* m_flameHitSE = nullptr;	//炎と当たったときのサウンド

			/*
			* 別のところで生成されるオブジェクト
			*/
			nsLevelObject::nsMobius::CMobius* m_mobius = nullptr;		//ステージのメビウスの輪のポインタ
			nsLight::CDirectionLight* m_gameDirectionLight = nullptr;	//ディレクションライトのポインタ
			nsCamera::CGameCamera* m_gameCamera = nullptr;				//ゲームカメラ
