		...
		namespace nsObstacleObject
		{
			...
			*/
			void OOTransparentSwitch::InitModel()
			{
				...
				m_pressedSwitchMR = NewGO<nsGraphic::nsModel::CModelRender>(PRIORITY_FIRST);
			...
			*/
			void OOTransparentSwitch::FindObjects()
			{
				...
				//ポーズを探す
				m_pause = FindGO<nsUI::CPause>(GetGameObjectName(EN_GO_TYPE_PAUSE));
			...
			void OOTransparentSwitch::UpdateSub()
			{
				...
				if (m_timerCountFlag)
				{
					...
					if (m_flagSwitchOn == true) {
						m_entityTimer -= nsTimer::GameTime().GetFrameDeltaTime();
			...
			*/
			...
			{
				//切り替え中タイマーを進める
				m_switchingTimer += nsTimer::GameTime().GetFrameDeltaTime();
				...
				else if (m_switchingTimer < TIME_FADE_WAIT_START)
				{
					...
					g_graphicsEngine->GetHUD().
						SetFadeSpriteAlphaValue(nsGraphic::nsSprite::spriteRenderConstData::ALPHA_VALUE_OPACITY);
			...
			*/
			void OOTransparentSwitch::Switching()
			{
				...
				if (m_switchingTimer < TIME_SWITCHING_HALF)
				{
					...
					//フェードを透明にする
					g_graphicsEngine->GetHUD().
						SetFadeSpriteAlphaValue(nsGraphic::nsSprite::spriteRenderConstData::ALPHA_VALUE_TRANSPARENT);
