		...
		*/
		namespace nsReversibleObject
		{
			...
			*/
			...
			{
				//南京錠が開くときのサウンドを生成する
				m_padlockbreakSE = NewGO<nsSound::CSoundCue>(nsCommonData::PRIORITY_FIRST);
			...
			void ROkey_padlock::QuerySub()
			{
				...
				if (GetObjectType() == EN_RO_TYPE_KEY)
				{
					...
					QueryLOs<nsObstacleObject::OOpadlock>(
						EN_OO_TYPE_BIG_PADLOCK, [&](nsObstacleObject::OOpadlock* bigPadlock) -> bool
						{
			...
			*/
			void ROkey_padlock::KeyHitBigPadlock(nsObstacleObject::OOpadlock* bigPadlock)
			{
				//行動できるか？
				if (m_actionFlag)
				{
