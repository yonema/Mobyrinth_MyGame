	...
	*/
	namespace nsUI
	{
		...
		bool CPause::Start()
		{
			//この関数限定で
			//スプライトを使用可能にする
			using namespace nsGraphic::nsSprite;

			//画像の表示
			m_level2D.Init("Assets/level2D/Pause_2.casl", [&](nsLevel2D::Level2DObjectData& objdata)
				{
					//名前が一致でフックする
					//カーソル
					if (objdata.EqualObjectName("Pause"))
					{
						m_pause = NewGO<CSpriteRender>(nsCommonData::PRIORITY_FOURTH);
					...
					{
						m_choices[enContinue] = NewGO<CSpriteRender>(nsCommonData::PRIORITY_FOURTH);
					...
					{
						m_choices[enRetry] = NewGO<CSpriteRender>(nsCommonData::PRIORITY_FOURTH);
					...
					{
						m_choices[enQuit] = NewGO<CSpriteRender>(nsCommonData::PRIORITY_FOURTH);
					...
					{
						m_cursor = NewGO<CSpriteRender>(nsCommonData::PRIORITY_FOURTH);
					...
					{
						m_background = NewGO<CSpriteRender>(nsCommonData::PRIORITY_THIRD);
						m_background->Init(
							objdata.ddsFilePath,
